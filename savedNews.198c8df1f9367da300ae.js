...
;// ./src/js/constants/savedNewsConsts.js
// saved-news title-section
var savedNewsTitleSectionTitle = document.querySelector('.title-section__title');
var savedNewsTitleSectionKeyword = document.querySelector('.title-section__description_bold');

// popup section
var popupSection = document.querySelector('.popup-section'); // Popups section

// saved-news header link logout
var savedNewsHeaderLinkSignout = document.querySelector('.header__link-logout');

// saved-news search-result card-container
var savedNewsCardContainer = document.querySelector('.search-result__card-container');

;// ./src/js/saved-news.js
...
function addcardnews(cards) {
  ...
  cards.forEach(function (element) {
    ...
    alertText.textContent = "".concat(element.keyword);
    ...
    cardMarkBtn.addEventListener('click', function (event) {
      mainapi.removeArticle(element._id, localStorage.getItem('token')).then(function () {
...
function keyword(massNews) {
  try {
    ...
    if (mass.length > 1) {
      for (var i = 0; i < mass.length; i++) {
        max[i] = 0;
        for (var g = 0; g < massNews.length; g++) {
    ...
    return {
      max1: mass[maxindex1],
      max2: mass[maxindex2],
      length: mass.length - 2
    };
  } catch (_unused) {
    throw new Error('Something wrong with keyword');
  }
}
document.querySelector('.header-section__mini-menu-open').addEventListener('click', function (event) {
  document.location.reload();
});
function titlesavearticles(element) {
  savedNewsTitleSectionTitle.textContent = "".concat(savedNewsHeaderLinkSignout.textContent, ", \u0443 \u0432\u0430\u0441 ").concat(element.length, " \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u043D\u044B\u0445 \u0441\u0442\u0430\u0442\u0435\u0439");
  if (keyword(element).max1 !== undefined && keyword(element).max2 !== undefined) {
    if (keyword(element).length !== 0) savedNewsTitleSectionKeyword.textContent = "\u041F\u043E \u043A\u043B\u044E\u0447\u0435\u0432\u044B\u043C \u0441\u043B\u043E\u0432\u0430\u043C: ".concat(keyword(element).max1, ", ").concat(keyword(element).max2, " \u0438 \u0435\u0449\u0451 ").concat(keyword(element).length, " \u0434\u0440\u0443\u0433\u0438\u043C");else savedNewsTitleSectionKeyword.textContent = "\u041F\u043E \u043A\u043B\u044E\u0447\u0435\u0432\u044B\u043C \u0441\u043B\u043E\u0432\u0430\u043C: ".concat(keyword(element).max1, ", ").concat(keyword(element).max2);
  }
  if (keyword(element).length === undefined && keyword(element).max1 !== undefined && keyword(element).max2 === undefined) savedNewsTitleSectionKeyword.textContent = "\u041F\u043E \u043A\u043B\u044E\u0447\u0435\u0432\u043E\u043C\u0443 \u0441\u043B\u043E\u0432\u0443: ".concat(keyword(element).max1);
  if (keyword(element).length === undefined && keyword(element).max1 === undefined && keyword(element).max2 === undefined) {
    savedNewsTitleSectionTitle.textContent = "".concat(savedNewsHeaderLinkSignout.textContent, ", \u0443 \u0432\u0430\u0441 \u043D\u0435\u0442 \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u043D\u044B\u0445 \u0441\u0442\u0430\u0442\u0435\u0439");
    savedNewsTitleSectionKeyword.textContent = 'Сохраненных статей нет!';
  }
}
function start() {
  savedNewsHeaderLinkSignout.addEventListener('click', function () {
    localStorage.removeItem('token');
    window.location.href = './';
  });
  mainapi.getUserData(localStorage.getItem('token')).then(function (res) {
    savedNewsHeaderLinkSignout.textContent = res.data.name;
  }).catch(function () {
    throw new Error('Something wrong with getUserData');
  });
  mainapi.getArticles(localStorage.getItem('token')).then(function (res) {
    titlesavearticles(res.data);
    addcardnews(res.data);
  }).catch(function () {
    throw new Error('Something wrong with getArticles');
  });
}
